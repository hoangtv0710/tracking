...
abstract class Base extends \Piwik\Plugin\Report
{
    ...
    protected function init()
    {
        ...
        if (property_exists($this, 'onlineGuideUrl')) {
            $this->onlineGuideUrl = 'https://matomo.org/docs/search-engine-keywords-performance/';
        }
    }
    ...
    protected function configureReportDiscontinued(ViewDataTable $view)
    {
        if (Date::today()->isLater(Date::factory('2019-03-30'))) {
            $view->config->show_footer_message .=
                '<p style="margin-top:2em;margin-bottom:2em" class=" alert-info alert">' .
                'Google discontinued the error samples API used to generate this report end of March. No new data can be imported. If you want to hide this report, you can disable it in the plugin settings.' .
                '</p>';
        }
    }
    ...
    {
        return $this->isGoogleEnabledForType('web') || $this->isGoogleEnabledForType('image') || $this->isGoogleEnabledForType('video');
    }
