...
class LogHsr
{
    ...
    public function install()
    {
        DbHelper::createTable($this->table, "
                  ...
                  `time_on_page` MEDIUMINT(8) UNSIGNED NOT NULL,
    ...
    protected function getDeviceType($hsrSiteIds, $idSite, $userAgent, $deviceWidth)
    {
        ...
        $factory = DeviceDetectorFactory::getInstance($userAgent);
        $device = $factory->getDevice();

        $checkWidth = false;
        if (in_array($device, array(DeviceParserAbstract::DEVICE_TYPE_FEATURE_PHONE, DeviceParserAbstract::DEVICE_TYPE_PHABLET, DeviceParserAbstract::DEVICE_TYPE_SMARTPHONE, DeviceParserAbstract::DEVICE_TYPE_CAMERA, DeviceParserAbstract::DEVICE_TYPE_CAR_BROWSER), $strict = true)) {
            $deviceType = self::DEVICE_TYPE_MOBILE;
        } elseif (in_array($device, array(DeviceParserAbstract::DEVICE_TYPE_TABLET), $strict = true)) {
            $deviceType = self::DEVICE_TYPE_TABLET;
        } elseif ($deviceType === DeviceParserAbstract::DEVICE_TYPE_DESKTOP) {
