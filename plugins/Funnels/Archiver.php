...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    public function aggregateDayReport()
    {
        ...
        if (empty($segment) || $segment->isEmpty()) {
            ...
            // dashboard
            $lock = 'funnels_populate_log_site_' . $idSite . (str_replace(array('-', '_', ' '), '', $startDateTime)) . '_' . (str_replace(array('-', '_', ' '), '', $endDateTime));
        ...
        foreach ($funnels as $funnel) {
            ...
            $referers->filterSubtables(function (DataTable $table) {
                // fix truncate may fail summing two strings
                $ops = $table->getMetadata(DataTable::COLUMN_AGGREGATION_OPS_METADATA_NAME);
                if (empty($ops)) {
                    $ops = array();
                }
                $ops['referer_type'] = function ($thisValue, $otherValue ) {
            ...
            $referers->filterSubtables('Truncate',
                array($this->maximumRowsInReferrers,
