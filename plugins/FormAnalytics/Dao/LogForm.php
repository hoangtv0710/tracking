...
class LogForm
{
    ...
    public function findRecordByIdView($idVisit, $idSiteForm, $idFormView)
    {
        ...
        $sql = sprintf('SELECT idlogform FROM %s FORCE INDEX (unique_idvisit_form_view) WHERE idvisit = ? AND idsiteform = ?  AND last_idformview = ? ORDER BY idlogform DESC LIMIT 1', $this->tablePrefixed);
    ...
    public function findUnconvertedRecord($idVisit, $idSiteForm)
    {
        ...
        // log_form entry
        $sql = sprintf('SELECT idlogform FROM %s FORCE INDEX (unique_idvisit_form_view) WHERE idvisit = ? AND idsiteform = ? AND converted = 0 ORDER BY idlogform DESC LIMIT 1', $this->tablePrefixed);
    ...
    public function addRecord($idVisitor, $idVisit, $idSite, $idSiteForm, $idFormView, $isViewed, $isStarted, $isSubmitted, $serverTime, $timeHesitation, $timeSpent, $timeToSubmission)
    {
        ...
        try {
            ...
            if (Db::get()->isErrNo($e, \Piwik\Updater\Migration\Db::ERROR_CODE_DUPLICATE_ENTRY)) {
                ...
                $this->updateRecord($idLogForm, $idFormView, $isViewed, $isStarted, $isSubmitted, $serverTime, $timeHesitation, $timeSpent, $timeToSubmission);
                return $idLogForm;
            }
            throw $e;
        }
